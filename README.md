...
- **Interactive Docs**: `http://localhost:8000/api/v1/schema/`
...
Current versioned API surface (`/api/v1/...`):

- `/api/v1/catalog/items/` – Inventory CRUD (CatalogItem + variants/media)
- `/api/v1/org/vendors/` – Vendors, memberships, and stores
- `/api/v1/auth/*` – Registration, JWT token/refresh, profile, password flows
- `/api/v1/schema/` – OpenAPI schema + docs (drf-spectacular UI)
- `/health/` – Deployment health probe
...
│   ├── catalog/          # Inventory domain (items, media, variants)
│   ├── org/              # Vendor, membership, store management
