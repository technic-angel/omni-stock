...
def create_test_image(filename="test.jpg", size=(100, 100), color="red"):
    ...
    return SimpleUploadedFile(name=filename, content=file.read(), content_type="image/jpeg")
...
def test_local_storage_saves_to_media_directory():
    ...
    user = User.objects.create_user(
        username="localuser", email="local@example.com", password="testpass123"
    )
...
def test_storage_backend_respects_upload_to_parameter():
    ...
    # ARRANGE
    user = User.objects.create_user(
        username="diruser", email="dir@example.com", password="testpass123"
    )
    profile, _ = UserProfile.objects.get_or_create(user=user)
    ...
    assert result.profile_picture.name.startswith(
        "profile_pictures/"
...
def test_storage_generates_unique_filenames():
    ...
    # ARRANGE
    user = User.objects.create_user(
        username="uniqueuser", email="unique@example.com", password="testpass123"
    )
    profile, _ = UserProfile.objects.get_or_create(user=user)
...
def test_supabase_storage_integration():
    ...
    # ARRANGE
    user = User.objects.create_user(
        username="supabaseuser", email="supabase@example.com", password="testpass123"
    )
    profile, _ = UserProfile.objects.get_or_create(user=user)
