...
The OpenAPI schema generated by `drf-spectacular` includes volatile fields (server URLs, timestamps, version info) that change between environments, causing false positives in CI schema drift detection.

## Solution

We normalize the schema before comparison by removing volatile fields and ensuring deterministic output.
